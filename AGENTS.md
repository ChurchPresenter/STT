...
- **Anything generated into a directory is staged too.** The CTranslate2 conversion builds under
  ...
  to a C++ reader.
- **Nothing the server inherits should be the watchdog's.** The graceful-shutdown channel is a pipe the watchdog holds as the server's stdin (`stt/watchdog.py`), and Windows has no cross-process SIGTERM, so losing it turns every stop into a kill with mid-write cuts to the session database. `stt/shutdown_channel.py` reads it from a private `dup` and leaves fd 0 on the null device, so children inherit nothing — and **every failure there falls back to reading the pipe in place**, because an inherited handle is a smaller problem than no shutdown at all. On Windows the CRT's fd 0 and the Win32 `STD_INPUT_HANDLE` children actually inherit are separate; `dup2` moves only the first.
- **A Windows process tree needs a job object.** `start_new_session` + `killpg` sweeps the tree on POSIX and is a documented no-op on Windows, where terminating the server orphaned its multiprocessing children (measured: four strays holding ~1.1 GB). `stt/win_job.py` binds the tree to a job with `JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE`; the `ProcessManager` holds that handle for the server's life and closing it kills whatever is left. Binding is allowed to fail — a leak beats a server that will not start.
